...
async function openDetails(projectId, trigger) {
  ...
  root.innerHTML = `<div class="detail-backdrop" data-action="close-detail"><aside class="detail-panel" role="dialog" aria-modal="true" aria-labelledby="detail-title">
    ...
    <div class="detail-body">
      ...
      ${section('2. 아이디어와 제작 목적', [field('만들고 싶은 작품', p.ideaDescription), field('주제 선정 이유', p.selectionReason), field('대상 사용자', p.targetUser), field('핵심 가치', listText(p.coreValues)), field('참고 작품', p.referenceProject), field('차별점', p.differentiation), ...(p.projectType === '생활 문제 해결형' ? [field('해결하고 싶은 문제', p.problemSituation),field('개선하고 싶은 내용',p.improvement)] : [])])}
